...
void ubpiontraj::PionMomentumLikelihood::beginJob()
{
   ...
   m_Tree->Branch("cleanliness", &m_cleanliness);
      ...
      if (abs(sim->PdgCode()) == 211 && (sim->Process() == "decay" || sim->Process() == "primary")) {
         ...
         for (const art::Ptr<recob::PFParticle>& reco : recoParticles) {
            ...
            for (const art::Ptr<recob::Track>& trk : recoTracks) {
               ...
               double matchedRecoEnergy = 0.0;

               std::vector<simb::MCParticle const*> depositingSimParticles;
               std::vector<anab::BackTrackerHitMatchingData const*> matchingHitData;
               simb::MCParticle const* matchedSimParticle = nullptr;

               for (size_t h = 0; h < hitVec.size(); ++h) {
                  m_RecoHitSimParticleAssoc->get(hitVec[h].key(), depositingSimParticles, matchingHitData);

                  for (size_t p = 0; p < depositingSimParticles.size(); ++p) {
                     trackMap[depositingSimParticles[p]->TrackId()] += matchingHitData[p]->energy;
                     totalRecoEnergy += matchingHitData[p]->energy;

                     if (depositingSimParticles[p]->TrackId() == sim->TrackId()) {
                        matchedRecoEnergy += matchingHitData[p]->energy;
                     }
                  }
               }

               if (matchedRecoEnergy == 0.0) continue;
               matchedSimParticle = depositingSimParticles[0];
               for (const auto& particle : depositingSimParticles) {
                  if (trackMap[particle->TrackId()] > trackMap[matchedSimParticle->TrackId()]) {
                     matchedSimParticle = particle;
                  }
               }
               ...
               if (totalRecoEnergy > 0) {
                  m_cleanliness = matchedRecoEnergy / totalRecoEnergy;
               }

               if (m_Debug) {
                  std::cout << "Cleanliness: " << m_cleanliness << std::endl;
               }
